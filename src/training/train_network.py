...
from src.experts.network_model import CNNLSTMClassifier
...
def parse_args() -> NetworkTrainConfig:
    parser = argparse.ArgumentParser(description="Train CNN-LSTM network anomaly expert.")
    ...
    parser.add_argument("--lstm-hidden-dim", type=int, default=96)
    parser.add_argument("--lstm-layers", type=int, default=1)
    parser.add_argument("--dropout", type=float, default=0.2)
    ns = parser.parse_args()
    return NetworkTrainConfig(
...
def main() -> None:
    ...
    train_loaded = load_processed_dataset(config.train_data, feature_dtype=torch.float32)
    val_loaded = load_processed_dataset(config.val_data, feature_dtype=torch.float32)
    test_loaded = (
        load_processed_dataset(config.test_data, feature_dtype=torch.float32)
        if config.test_data is not None
        else None
    )

    if train_loaded.features.ndim != 3:
        raise ValueError(
            f"Network data must be [N, seq_len, feature_dim], got {tuple(train_loaded.features.shape)}"
        )
    input_dim = int(train_loaded.features.shape[-1])
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        save_model_checkpoint(
            ...
            config=_model_config_payload(config=config, input_dim=input_dim, num_classes=num_classes),
            epoch=epoch,
